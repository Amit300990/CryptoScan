  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        // In dev the Replit internal proxy sets x-forwarded-host to "localhost",
        // not the real external domain. Use REPLIT_DEV_DOMAIN when available so
        // Clerk's FAPI redirects back to the correct public URL.
        let proxyUrl: string;
        if (process.env.REPLIT_DEV_DOMAIN) {
          proxyUrl = `https://${process.env.REPLIT_DEV_DOMAIN}${CLERK_PROXY_PATH}`;
        } else {
